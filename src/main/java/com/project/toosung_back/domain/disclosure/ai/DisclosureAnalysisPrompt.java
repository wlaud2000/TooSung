...
public class DisclosureAnalysisPrompt {
    ...
    private static final int RAW_DATA_MAX_LENGTH = 3500;
    ...
    private static final String EARNINGS_SYSTEM_PROMPT = """
            ...
            {
              "simpleSummary": "이 실적 보고서의 내용을 일반 투자자가 이해할 수 있는 언어로 상세히 설명",
              "investmentPoint": "매출·영업이익 증감 방향과 주가에 미치는 영향을 2문장으로 설명"
            }

            규칙:
            - simpleSummary: 매출·영업이익·순이익 등 주요 수치를 구체적으로 언급하고, 전년 대비 변화와 그 원인을 쉬운 말로 상세히 설명. 쉼표(,)로 이어질 수 있는 내용은 각각 별도 문장으로 분리해 짧게 작성. 문장의 개수는 제한 없음
            - investmentPoint: 전년 동기 대비 증감 방향, 수익성 변화, 주가 영향 방향(긍정/부정/중립)을 포함해 2문장으로 작성
            """;
    ...
    private static final String CAPITAL_SYSTEM_PROMPT = """
            ...
            {
              "simpleSummary": "이 공시의 내용을 일반 투자자가 이해할 수 있는 언어로 상세히 설명",
              "investmentPoint": "주식 수 증가(희석 효과)와 기존 주주 가치에 미치는 영향을 2문장으로 설명"
            }

            규칙:
            - simpleSummary: 조달 규모·목적·방식·일정을 구체적인 수치와 함께 상세히 설명. 쉼표(,)로 이어질 수 있는 내용은 각각 별도 문장으로 분리해 짧게 작성. 문장의 갯수는 제한 없음
            - investmentPoint: 신주 발행에 따른 주식 수 증가율(희석 효과), 기존 주주 가치 훼손 여부, 조달 목적의 적정성을 포함해 2문장으로 작성
            """;
    ...
    private static final String RESTRUCTURING_SYSTEM_PROMPT = """
            ...
            규칙:
            - simpleSummary: 합병·분할 상대방, 목적, 규모, 일정, 조건을 구체적으로 상세히 설명. 쉼표(,)로 이어질 수 있는 내용은 각각 별도 문장으로 분리해 짧게 작성. 문장의 개수는 제한 없음
            - investmentPoint: 합병 시너지 또는 분할 후 사업 집중도 변화, 주가 영향 방향(긍정/부정/중립)을 포함해 2문장으로 작성
            """;
    ...
    private static final String TREASURY_SYSTEM_PROMPT = """
            ...
            규칙:
            - simpleSummary: 취득·처분 규모, 목적, 기간, 방법, 대상 인원 등을 구체적인 수치와 함께 상세히 설명. 쉼표(,)로 이어질 수 있는 내용은 각각 별도 문장으로 분리해 짧게 작성. 문장의 개수는 제한 없음
            - investmentPoint: 주가 방어 효과(취득) 또는 희석 영향(처분), 시장에 보내는 신호(경영진 자신감 등)를 포함해 2문장으로 작성
            """;
    ...
    private static final String DEFAULT_SYSTEM_PROMPT = """
            ...
            규칙:
            - simpleSummary: 공시의 구체적인 내용(수치, 일정, 관련자 등)을 빠짐없이 쉬운 말로 상세히 설명. 쉼표(,)로 이어질 수 있는 내용은 각각 별도 문장으로 분리해 짧게 작성. 문장의 개수는 제한 없음
            - investmentPoint: 주가 영향 방향(긍정/부정/중립)과 투자자가 주목해야 할 포인트를 포함해 2문장으로 작성
            """;
